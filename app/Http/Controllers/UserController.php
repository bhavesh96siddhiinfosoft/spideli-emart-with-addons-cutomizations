...
class UserController extends Controller
{
    ...
    public function storeAdminUsers(Request $request)
    {
        ...
        AdminUserRegion::syncUserRegions($user->id, $request->input('regions', []));
    ...
    public function editAdminUsers($id)
    {
        ...
        $user_regions = AdminUserRegion::regionsOfUser($id);
        return view('admin_users.edit', compact(['user', 'roles', 'user_regions']));
    ...
    public function updateAdminUsers(Request $request, $id)
    {
        ...
        if ($user) {
            ...
            if (!AdminUserRegion::isSuperAdmin($id)) {
                AdminUserRegion::syncUserRegions($id, $request->input('regions', []));
            }
        }
    ...
    public function deleteAdminUsers($id)
    {
        ...
        if (is_array($id)) {
            ...
            for ($i = 0; $i < count($id); $i++) {
                ...
                AdminUserRegion::syncUserRegions($id[$i], []);
                $users->delete();
            }

        } else {
            $user = User::find($id);
            AdminUserRegion::syncUserRegions($id, []);
            $user->delete();
        }
